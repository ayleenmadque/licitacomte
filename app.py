...
def guardar_en_supabase(resultados):
    ...
    try:
        get_supabase().table("licitaciones").upsert(
            filas,
...
st.markdown(
    '<div class="subtitle">Sistema de inteligencia de licitaciones — Perfil: COMTE</div>',
    unsafe_allow_html=True
)
...
if seccion == "Oportunidades":
    ...
    if not postulaciones:
        ...
        columnas_mostrar = [
            "nombre", "organismo", "region", "estado",
            "monto_estimado", "monto_ofertado",
        ...
        if filas_crm:
            ...
            with col_a:
                nuevo_estado = st.selectbox(
                    "Estado",
                    ESTADOS,
                    index=ESTADOS.index(registro["estado"])
                )

                monto_ofertado = st.number_input(
                    "Monto ofertado ($)",
                    value=int(registro.get("monto_ofertado") or 0),
                    step=100000
                )
            ...
            with col_b:
                monto_adjudicado = st.number_input(
                    "Monto adjudicado ($)",
                    value=int(registro.get("monto_adjudicado") or 0),
                    step=100000
                )
    ...
    if fila:
        ...
        if not datos:
            st.warning("No hay datos históricos similares aún. El agente los cargará en la próxima corrida.")
        else:
            ...
            if metricas:
                ...
                columnas_hist = [
                    "nombre", "organismo", "region",
                    "monto_adjudicado", "empresa_adjudicada",
